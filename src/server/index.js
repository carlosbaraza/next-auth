...
export default async (req, res, userSuppliedOptions) => {
  ...
  return new Promise(async (resolve) => {
    ...
    const { nextauth, action = nextauth[0], provider = nextauth[1], error = nextauth[1] } = query;
    ...
    if (req.cookies[cookies.csrfToken.name]) {
      ...
      if (
        csrfTokenHash === createHash("sha256").update(`${csrfTokenValue}${secret}`).digest("hex")
      ) {
      ...
      switch (action) {
        ...
        case "error":
          if (options.pages.error) {
            return redirect(
              `${options.pages.error}${options.pages.error.includes("?") ? "&" : "?"}error=${error}`
            );
          }
